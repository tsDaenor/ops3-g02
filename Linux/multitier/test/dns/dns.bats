#! /usr/bin/env bats
#
# Acceptance test for the DNS server for ops3-g02
...
assert_forward_lookup() {
  ...
  [ "$ip" = "$(dig @${sut_ip} ${name}.${domain} +short | grep $ip)" ]
}
...
@test 'Forward lookup DNS server' {
  #                     host name  IP
  assert_forward_lookup dns        192.0.2.52
}

@test 'Forward lookup Database server' {
  #                     host     name  IP
  assert_forward_lookup database       192.0.2.50
}

@test 'Forward lookup Monitoring server' {
  #                     host     name  IP
  assert_forward_lookup monitor        192.0.2.51
}

@test 'Forward lookups Web servers' {
  #                        host name  IP
  assert_forward_lookup    wp         192.0.2.53
  assert_forward_lookup    wp         192.0.2.54
  assert_forward_lookup    wp         192.0.2.55
}

@test 'Reverse lookup DNS server' {
  #                     host name  IP
  assert_reverse_lookup dns        192.0.2.52
}

@test 'Reverse lookup Database server' {
  #                     host     name  IP
  assert_reverse_lookup database       192.0.2.50
}

@test 'Reverse lookup Monitoring server' {
  #                     host     name  IP
  assert_reverse_lookup monitor        192.0.2.51
}

@test 'Reverse lookups Web servers' {
  #                     host name  IP
  assert_reverse_lookup wp         192.0.2.53
  assert_reverse_lookup wp         192.0.2.54
  assert_reverse_lookup wp         192.0.2.55
}
